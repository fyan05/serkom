...
<div class="container">
    ...
    <a href="{{ route('operator.siswa.create') }}" class="btn btn-primary mb-3">+ Tambah Siswa</a>
                ...
                <tbody>
                    @forelse($siswa as $key => $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('operator.siswa.edit', Crypt::encrypt($item->id)) }}" class="btn btn-warning btn-sm">Edit</a>
                                <form action="{{ route('operator.siswa.delete', $item->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
